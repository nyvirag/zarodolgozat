...
import Korocske from './Korocske'
import { createBottomTabNavigator } from '@react-navigation/bottom-tabs';
import Keszitok from './Keszitok'
import probaLogin from './probaLogin'
  ...
  return (
   <GestureHandlerRootView style={{flex:1}}>
    <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' ,backgroundColor:colors.black }}>
      ...
      <Pressable onPress={() => navigation.navigate('Étrendek')} style={({ pressed }) => ({
      ...
      <Pressable onPress={() => navigation.navigate('Ferfi')} style={({ pressed }) => ({
        backgroundColor: pressed ? colors.black : colors.sotetlime,
        elevation: pressed ? 2 : 0,
        borderRadius: 10, 
        marginBottom:10, 
         })} >
        <Text style={{fontSize:25, color:colors.black, padding:10, paddingHorizontal:54 }} > Férfi Étrend</Text>
      </Pressable>
...
function Vissza({ navigation }) {
  return (
    <Drawer.Navigator initialRouteName="Home" screenOptions={{
      ...
      <Drawer.Screen name="login" component={probaLogin} />
...
export default function App() {
  return (
    ...
    tabBarStyle: {
        ...
        <Tab.Screen name="Lepes" component={FerfiEtrend} options={{headerShown:false,  tabBarIcon: ({ color }) => (
